...
\def\y{0.92}
\def\off{1.25}
\node[nodea, minimum width=1.1cm, minimum height=1.1cm] at (-7.0, 0) {\small{geqrt}};
\node[nodeb, minimum width=1.1cm, minimum height=1.1cm] at (-7.0,-1.5) {\small{tpqrt}};
\node[nodec, minimum width=1.1cm, minimum height=1.1cm] at (-7.0,-3.0) {\small{gemqrt}};
\node[noded, minimum width=1.1cm, minimum height=1.1cm] at (-7.0,-4.5) {\small{tpmqrt}};
...
\end{center}
\end{frame}
% and ( 0.00,-3.5*\y + \off) and ( 1.25,-3.5*\y + \off) and ( 2.50,-3.5*\y + \off)
...
\begin{center}
\begin{tikzpicture}
\begin{axis}[name=plot1,height=6.5cm,width=13cm,xlabel={Threads \(T\)},ylabel={Runtimes Ratio [1]},legend style={font=\tiny}]
        \addplot[color=fzjorange,mark=+]
            coordinates
            {
